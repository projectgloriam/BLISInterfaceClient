 ...
 */
public class SYSMEXXP300 extends Thread{
    ...
    public void Stop()
    {
        try {
            ...
            welcomeSocket.close();
//            connSock.close();
             log.AddToDisplay.Display("SYSMEX XP-300 handler stopped", DisplayMessageType.TITLE);
      ...
      @Override
    public void run() {
        log.AddToDisplay.Display("SYSMEX XP-300 handler started...", DisplayMessageType.TITLE);
        ...
        try
        {
  		...
  		log.AddToDisplay.Display("Listening on port "+ tcpsettings.PORT+"...",DisplayMessageType.INFORMATION);
                ...
                log.AddToDisplay.Display("SYSMEX XP-300 is now Connected...",DisplayMessageType.INFORMATION);
                first=false;
                ClientThread client = new ClientThread(connSock,"SYSMEX XP-300");
                client.start();
                String message ;
		...
		{
                    log.AddToDisplay.Display("SYSMEX XP-300 client is now disconnected!",DisplayMessageType.WARNING);
                    log.logger.Logger(e.getMessage());
		}
         ...
         try
         {
            ...
            {            
                ...
                {               
                       ...
                       log.AddToDisplay.Display("Sending test with Code: "+SampleList.get(i).aux_id + " to SYSMEX XP-300",DisplayMessageType.INFORMATION);
    ...
    {  
        try
        {
            ...
            {
                //todo handle results
                String pidParts[] = msgParts[2].split("\\|")[3].split("\\^");
                if(pidParts.length > 2)
                {
                    String SampleID =  pidParts[2].trim();
                    int mID=0;
                    float value = 0;
                    boolean flag = false;
                    for(int i=3;i<22;i++)
    ...
    {
        String data = "";
        xmlparser p = new xmlparser("configs/SYSMEX/SYSMEXXP300.xml");
        try {
            data = p.getMicros60Filter(whichdata);           
        } catch (Exception ex) {
            Logger.getLogger(SYSMEXXP300.class.getName()).log(Level.SEVERE, null, ex);
        }        
        return data;        
    }
