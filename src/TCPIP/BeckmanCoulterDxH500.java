 ...
 */
public class BeckmanCoulterDxH500 extends Thread{
    ...
    public void Stop()
    {
        try {
            ...
            if(null !=welcomeSocket)
                 welcomeSocket.close();
//            connSock.close();
             log.AddToDisplay.Display("Beckman Coulter DxH500 handler stopped", DisplayMessageType.TITLE);
    ...
    public void run() {             
        ...
        try
        {
            log.AddToDisplay.Display("Beckman Coulter DxH500 handler started...", DisplayMessageType.TITLE); 
  		...
  		log.AddToDisplay.Display("Listening on port "+ tcpsettings.PORT+"...",DisplayMessageType.INFORMATION);
            ...
            else
            {
               ...
               connSock = new Socket(tcpsettings.EQUIPMENT_IP, tcpsettings.PORT);
                ...
                log.AddToDisplay.Display("Beckman Coulter DxH500 is now Connected...",DisplayMessageType.INFORMATION);
                first=false;
                if(!tcpsettings.SERVER_MODE)
                {
                    connSock.setKeepAlive(true);
                }
                ClientThread client = new ClientThread(connSock,"Beckman Coulter DxH500");
                client.start();
                String message ;
		...
		{
                    log.AddToDisplay.Display("Beckman Coulter DxH500 client is now disconnected!",DisplayMessageType.WARNING);
                    log.logger.Logger(e.getMessage());
		}
        ...
        /* if(!tcpsettings.SERVER_MODE)
               {
                 ...
                 try {
                     ...
                     Logger.getLogger(BeckmanCoulterDxH500.class.getName()).log(Level.SEVERE, null, ex);
                 }
                   MainForm.btobj = new BeckmanCoulterDxH500();
         ...
         try
         {
            ...
            {            
                ...
                {     
                       ...
                       log.AddToDisplay.Display("Sending test with Code: "+SampleList.get(i).aux_id + " to Beckman Coulter DxH500",DisplayMessageType.INFORMATION);
   ...
   private static void prepare(sampledata get)
   {
    ...
    {
        String data = "";
        xmlparser p = new xmlparser("configs/Beckman Coulter/BeckmanCoulterDxH500.xml");
        try {
