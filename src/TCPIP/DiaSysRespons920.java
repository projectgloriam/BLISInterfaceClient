 ...
 */
public class DiaSysRespons920 extends Thread{
    ...
    public void Stop()
    {
        try {
            ...
            if(null !=welcomeSocket)
                 welcomeSocket.close();
//            connSock.close();
             log.AddToDisplay.Display("DiaSys Respons 920 handler stopped", DisplayMessageType.TITLE);
    ...
    public void run() {             
        ...
        try
        {
            log.AddToDisplay.Display("DiaSys Respons 920 handler started...", DisplayMessageType.TITLE); 
  		...
  		log.AddToDisplay.Display("Listening on port "+ tcpsettings.PORT+"...",DisplayMessageType.INFORMATION);
            ...
            else
            {
               ...
               connSock = new Socket(tcpsettings.EQUIPMENT_IP, tcpsettings.PORT);
                ...
                log.AddToDisplay.Display("DiaSys Respons 920 is now Connected...",DisplayMessageType.INFORMATION);
                first=false;
                if(!tcpsettings.SERVER_MODE)
                {
                    connSock.setKeepAlive(true);
                }
                ClientThread client = new ClientThread(connSock,"DiaSys Respons 920");
                client.start();
                String message ;
		...
		{
                    log.AddToDisplay.Display("DiaSys Respons 920 client is now disconnected!",DisplayMessageType.WARNING);
                    log.logger.Logger(e.getMessage());
		}
            ...
            {            
                ...
                { 
                    appMode = MODE.SENDING_QUERY;                    
                       log.AddToDisplay.Display("Sending test with Code: "+SampleList.get(i).aux_id + " to DiaSys Respons 920",DisplayMessageType.INFORMATION);
   ...
   private static void noDataResponse()
   {
       ...
       strTemp.append("||BLIS|||||DiaSys Respons 920^DiaSys Respons 920^4.3||P|1394-97|");       
   ...
   private static void prepare(sampledata get)
   {
      ...
      private static String getCorrectSpecimenID(String raw)
    {
        ...
        {
           ID = utilities.getSystemDate("yyyyMMdd") + raw;           
        }
        else
        {
            ID = raw;
           ...
           if(!ASTMMsgs.isEmpty())
               {
                   ...
                   ASTMMsgs = ASTMMsgs.replaceAll(String.valueOf(ETB)+String.valueOf(CR), "");
                    ...
                    for (String msgPart: msgParts) {
                        ...
                        if (msgPart.startsWith("O")) {

                            SampleID = msgPart.split("\\|")[2].trim();
                            SampleID = getCorrectSpecimenID(SampleID);
                        } else if (msgPart.startsWith("R")) {
                            mID = getMeasureID(msgPart.split("\\|")[2].split("\\^")[3]);
                            
                            if(mID > 0)
                            {
                                try
                                {
                                    value = msgPart.split("\\|")[3].trim();
    ...
    {
        String data = "";
        xmlparser p = new xmlparser("configs/DiaSys/DiaSysRespons920.xml");
        try {
